...
# Создание неразрушимого кирпича
indestructible_brick = (100, 100, brick_width, brick_height)
indestructible_brick_color = (255, 0, 0)  # Красный цвет для неразрушимого кирпича
bricks.append({"rect": indestructible_brick, "color": indestructible_brick_color, "indestructible": True})

for i in range(2):
    for j in range(10):
        brick_rect = pygame.Rect(j * (brick_width + 10) + 25, i * (brick_height + 10) + 5, brick_width, brick_height)
        bricks.append({"rect": brick_rect, "color": brick_color, "indestructible": False})
...
while run:
    ...
    for brick in bricks:
        if ball_rect.colliderect(pygame.Rect(brick["rect"])):
            if not brick["indestructible"]:  # Проверка, является ли кирпич неразрушимым
                bricks.remove(brick)
    ...
    for brick in bricks:
        pygame.draw.rect(win, brick["color"], brick["rect"])
